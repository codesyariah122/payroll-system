<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Payroll</h2>
    </x-slot>
            ...
            <div class="rounded-xl bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <form action="{{ route('admin.payrolls.update', $payroll) }}" method="POST">
                    ...
                    <div class="grid gap-6 lg:grid-cols-2">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Periode
                                Payroll</label>
                            <input type="text" name="period" value="{{ old('period', $payroll->period) }}"
                                class="mt-2 block w-full rounded-lg border-gray-300 bg-white text-gray-900 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100"
                                required>
                            @error('period')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        ...
                        <button type="submit"
                            class="inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">Perbarui
                            Payroll</button>
                    </div>
                </form>
            </div>
