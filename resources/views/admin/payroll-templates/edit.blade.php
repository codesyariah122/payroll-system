<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Template Slip</h2>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ $template->name }}</p>
            </div>
        ...
        <div class="mx-auto max-w-6xl sm:px-6 lg:px-8">
            <div class="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-2xl backdrop-blur-xl">
                @include('admin.payroll-templates.form', [
                    'action' => route('admin.payroll-templates.update', $template),
                    'method' => 'PUT',
                    'template' => $template,
